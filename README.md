...
また、ドメインレイヤーのパッケージ内に、Issuesオブジェクトを取得するためのIssuesFetcherインターフェースを定義し、インフラストラクチャーレイヤー内でそのインターフェースを実装するようにしています。
...
GitHub APIの仕様上一度のリクエストで100件までしか情報を取得出来ないため、本APIは**open状態のissueのうち、最近作成されたissueを最大で100件を返却する**仕様となっています。
...
* HttpRequestのログについて、bodyの内容が出力出来ていない。
  * Java 11のHttp Client実装を使用しているが、Bodyの情報を格納するためのビルトインオブジェクト(BodyPublisher)がbodyの内容のStringとして返却するメソッドを持っていないため。
* Htmlタグなどが含まれる可能性のある外部サイトからデータを取ってきているため、本APIのResponseのHeaderとして`X-Content-Type-Options: nosniff`をつけたほうがよい。
  * 調査不足だが、Spring Securityなどの機能を使うと、各endpointへ一括で当該Headerをレスポンスに加えられるはず。
